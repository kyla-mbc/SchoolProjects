...
class Character:
    ...
    def calculate_damage(self, attacker, defender, is_special_attack=False):
        ...
        # Check for maximum and minimum damage
        if base_damage == attack_dice:
            print(f"\t MAXIMUM POSSIBLE DAMAGE DONE TO {defender.player_type}")
        elif base_damage == 1:
            print(f"\t LOWEST POSSIBLE DAMAGE DONE TO {defender.player_type}")

        if base_damage >= armor_class:
            base_damage += attack_modifier

        return base_damage, attack_dice
...
# NPC and PC Types
PCtypes = {
    "PC01": Character("Warlock", 90, 10, 24, 15, 25, 3),
    "PC02": Character("Ranger", 70, 10, 22, 15, 25, 3),
    "PC03": Character("Wizard", 80, 15, 25, 15, 25, 3),
    "PC04": Character("Barbarian", 100, 21, 19, 25, 10, 3),
    "PC05": Character("Cleric", 80, 15, 23, 15, 25, 3)
}

NPCtypes = {
    "NPC1": Character("Sorcerer", 100, 15, 23, 20, 25, 3),
    "NPC2": Character("Paladin", 80, 10, 25, 20, 25, 3),
    "NPC3": Character("Rogue", 60, 20, 21, 15, 25, 3)
}
    ...
    while PCstats.health > 0 and NPCstats.health > 0:
        ...
        if option.lower() == "attack":
            ...
            if npc_choice == "Attack":
                ...
                NPC_damage = PCstats.calculate_damage(NPCstats, PCstats, True)
                player_damage = PCstats.calculate_damage(PCstats, NPCstats)
                if player_damage < NPCstats.ac:
                    print("\t You missed!")
                elif player_damage >= NPCstats.ac:
            ...
            if PCstats.perform_special_attack(NPCstats):
                if npc_choice == "Attack":
                    print(f"\t The {NPC} attacked you!")
                    player_damage = PCstats.calculate_damage(PCstats, NPCstats, True)
                    NPC_damage = NPCstats.calculate_damage(NPCstats, PCstats)
                    if player_damage < NPCstats.ac:
                        print("\t You missed!")
                    elif player_damage >= NPCstats.ac:
                ...
                if option.lower() == "attack":
                    ...
                    if npc_choice == "Attack":
                        ...
                        print(f"\t The {NPC} used Special Attack!")
                        NPC_damage = PCstats.calculate_damage(NPCstats, PCstats)
                        player_damage = PCstats.calculate_damage(PCstats, NPCstats)
                        if player_damage < NPCstats.ac:
                            print("\t You missed!")
                        elif player_damage >= NPCstats.ac:
