...
class Character:
    ...
    def calculate_damage(self, attacker, defender, is_special_attack=False, action=None):
        if action == "Dodge":
            print(f"\t {defender.player_type} chose to dodge!")
            return 0
...
PCtypes = {
    ...
    "PC04": Character("Barbarian", 100, 21, 20, 20, 25, 3),
...
while True:
    ...
    if transcript_file_path == "" or ".txt" not in transcript_file_path:
...
with open(transcript_file_path, "a") as transcript:
    ...
    while PCstats.health > 0 and NPCstats.health > 0:
        ...
        if option.lower() == "dodge" or npc_choice == "Dodge":
            if option.lower() == "dodge" and npc_choice == "Dodge":
                transcript.write(f"\tBoth players dodged.\n")
            elif option.lower() == "dodge":
                transcript.write(f"\tYou dodged.\n")
            elif npc_choice == "Dodge":
                transcript.write(f"\tThe {NPC} dodged.\n")
        else:
            if player_damage < NPCstats.ac:
                transcript.write(f"\t{PLAYER} missed.\n")
            elif player_damage >= NPCstats.ac:
                transcript.write(f"\t{PLAYER}'s damage done: {player_damage}\n")
            if NPC_damage < PCstats.ac:
                transcript.write(f"\tThe {NPC} missed.\n")
            elif NPC_damage >= PCstats.ac:
