...
/*
    ...
    function that takes a matrix of integers (as a pointer to an array of pointers)
     ...
     Remember to free the allocated memory before the program exits to avoid memory leaks.
...
int main(int argc, char* argv[]) { //EDIT MAIN METHOD TO ALLOW COMMAND LINE ARGS
    ...
    int rows, cols;
    if (argv >= 3){
        std::cout << "file name: " << argv[0] << "\n";
        rows = std::atoi(argv[1]); // takes the array of chars and turns it into integers
